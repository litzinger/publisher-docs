...
Alternatively generate an <ul> of linked languages. Use this parameter to get up and running quickly. It will immediately let you change languages with no addtional JavaScript.
...
{exp:publisher:translate_phrase name="some_word"}
...
{/exp:publisher:phrases}
Searching
Publisher works with the native ExpressionEngine search module, but only when using EE 2.8+.

If using EE 2.6, or 2.7, you can use the following module tag to assist in your entries search.
...
{exp:channel:entries publisher_cache="yes"}
    ...
{/exp:channel:entries}
{exp:channel:prev/next_entry} tags
